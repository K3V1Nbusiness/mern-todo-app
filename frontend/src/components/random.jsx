...
function TodoList({ todos, setTodos, fetchtodos }) {
  ...
  function handleSearch(value) {
    if (!value) {
      // If search query is empty, fetch all todos again
      fetchtodos();
      return;
    }

    // Filtering todos based on title and description
    const objKeys = ["title", "description"];
    const filteredTodos = todos.filter((todo) =>
      objKeys.some((key) => todo[key]?.toLowerCase().includes(value.toLowerCase()))
    );
      ...
      <div className="flex space-x-4 mt-10">
        <div className="flex rounded-md overflow-hidden min-w-[80%] border border-indigo-600">
          <input
            ...
            onChange={handleInputChange} // Directly call handleInputChange
          />
          <button className="text-red-500 px-5 text-lg font-semibold py-2 rounded-r-md">
